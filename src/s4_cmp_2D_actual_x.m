...
if isAPP
    ...
    radialIncrement = 'Surface'; % 'Surface' or 'On-Axis'
    aimRes = 0.5; % um
    xIncrement = 24.5874; % um
    maxIter = 100;
...
msgfig = questdlg({sprintf(['\\fontsize{%d}\\fontname{%s}', ...
    ...
    sprintf('4. X increment: %s',radialIncrement), ...
...
% assign space
if strcmp(radialIncrement,'On-Axis')
    discretizationLength = abs(rRange(2) - rRange(1));
else
    syms xx yy;
    curveFxSym = subs(surfFx,y,0);
    curveArcSym = int((1 + curveFxSym.^2).^0.5,x,xx,yy);
    curveArc = matlabFunction(curveArcSym,'Vars',{xx,yy});
    discretizationLength = curveArc(rRange(1),rRange(2));
end
curveNum = ceil(discretizationLength/xIncrement) + 1;
curveULim = cell(1,curveNum);
curveRes = 5*aimRes*ones(1,curveNum);
% rStep = rStepDir*discretizationLength/(curveNum - 1);
rStep = rStepDir*xIncrement;
...
for ind = 2:curveNum
    tic;
    % rStep determination
    r = curvePt(1,ind - 1) + rStep;
    if strcmp(radialIncrement,'Surface')
        r = solvechord( ...
            matlabFunction(subs(curveArcSym,xx,curvePt(1,ind - 1)) - rStep), ...
            r,r + 10,1e-5,maxIter);
    end

    curvePt(:,ind) = [r;0;curveFunc(r)];
    curveNorm = [curveFx(r);0;-1];
    curveNorm = curveNorm./norm(curveNorm);
    [curvePathPt(:,ind),curveQuat(ind,:),curveContactU(ind)] = ...
        curvetippos(toolData,curvePt(:,ind),curveNorm,[0;0;-1],cutDirect, ...
...
curvePathPt(1,curveNum) = 0;
[curvePathPt(:,end),curveQuat(end,:),curveContactU(end),curvePt(:,end)] = ...
    curvepos(curveFunc,curveFx,toolData,curvePathPt(:,end),[0;0;-1],cutDirect, ...
    'directionType','norm-cut');
% calculate the residual height of the loop and the inner nearest loop
toolSp1 = toolSp;
...
prevUlimInd = size(curveULim{end},2); % get the curveUlim use of the last pathpt
if prevUlimInd > 1
    curveULim{end - 1}(:,(end - prevUlimInd + 2):end) = [];
end
switch uDirection
    case 'U Plus'
        curveULim{end - 1}(end) = 1;
        curvePlotSpar = 0.0001;
    case 'U Minus'
        curveULim{end - 1}(end) = 0;
        curvePlotSpar = -0.0001;
end
if dist2Surf
...
switch approxMethod
    ...
    set(gca,'FontSize',textFontSize,'FontName',textFontType,'XAxis',);
    grid on;
