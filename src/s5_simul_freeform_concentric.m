...
% concentric surface point (proved to be wrong)
% r = 0:(toolData.radius/2):rMax;
% for ii = 1:length(r)
...
% concentric tool path point (answered by Prof. Huang & Doct. Chen)
r = 0:(toolData.radius/2):rMax;
toolPathX = [];
toolPathY = [];
parfor ii = 1:length(r)
    ...
    conTheta(end) = [];
    sparTheta(ii) = length(conTheta);
    toolPathX = [toolPathX,r(ii)*cos(conTheta)];
    toolPathY = [toolPathY,r(ii)*sin(conTheta)];
end
ptNum = length(toolPathX);
toolPathPt(1:2,:) = [toolPathX;toolPathY];


for kk = 1:ptNum
    surfPos()
end
toolPathPt(3,:) = surfFunc(surfPt(1,:),surfPt(2,:));
