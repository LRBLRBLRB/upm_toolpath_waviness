...
if isTest
    ...
    plot(toolOri(:,1),toolOri(:,2),'.','Color',[0,0.45,0.74]); hold on;
    % plot the fitting center of the circle
    scatter(fitCirc{1}(1),fitCirc{1}(2),36,[0.6350,0.0780,0.1840],'filled');
    arcVec = 0.5*(fitCirc{4} + fitCirc{5});
    quiver(fitCirc{1}(1),fitCirc{1}(2),arcVec(1),arcVec(2), ...
    ...
    R = rotz(atan2(arcVec(2),arcVec(1)) - 0.5*fitCirc{3});
