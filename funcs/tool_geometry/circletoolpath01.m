function r = circletoolpath01(r0,toolData,delta,surfFunc,surfNormFunc, ...
    angularDiscrete,conThetaBound,maxAngPtDist,arcLength,angularLength, ...
    aimRes,rMax,tRes0,conTheta1)
...
%     while true
        ...
        for indTmp = 1:loopPtNumTmp
            [toolQuatTmp(indTmp,:),toolVecTmp(:,indTmp),toolContactUTmp(indTmp)] = toolPos( ...
        ...
        for indIn1 = loopPtNumLast + 1:loopPtNumLast + loopPtNumTmp
            ...
            [resTmp(1,indIn1),peakPtInTmp(:,indIn1),uLimTmp(:,indIn1)] = residual3D( ...
                ...
                toolSp,toolRadius,uLimTmp(:,indIn1),indIn1,indIn2,indIn3);
...
%         figure;
%         for ii = 1:loopPtNumLast + loopPtNumTmp
%             toolSp1 = toolSp;
%             toolSp1.coefs = ...
%                 axesRot([0;0;1],[1;0;0],toolNormDirectRes(:,ii),toolCutDirectRes(:,ii),'zx') ...
%                 *toolSp.coefs + toolPathPtRes(:,ii);
%             Q = fnval(toolSp1,uLimTmp(1,ii):0.002:uLimTmp(2,ii));
%             plot3(Q(1,:),Q(2,:),Q(3,:),'r');
%             hold on;
%         end
...
% figure;
% for indIn = loopPtNumLast + 1:loopPtNumLast + loopPtNumTmp
%     toolSp1 = toolSp;
%     toolSp1.coefs = ...
%         axesRot([0;0;1],[1;0;0],toolNormDirectRes(:,indIn),toolCutDirectRes(:,indIn),'zx') ...
%         *toolSp.coefs + toolPathPtRes(:,indIn);
%     Q = fnval(toolSp1,uLimTmp(1,indIn):0.01:uLimTmp(2,indIn));
%     plot3(Q(1,:),Q(2,:),Q(3,:),'r');
%     hold on;
% end

for indOut1 = 1:loopPtNumLast
    angleDel = angleOut - angle(indOut1);
    [indOut2,indOut3] = getOuterLoopToolPathIndex(angleOut,angleDel,loopPtNumLast);

%     toolSp1 = toolSp;
%     toolSp1.coefs = ...
%         axesRot([0;0;1],[1;0;0],toolNormDirectRes(:,indOut1),toolCutDirectRes(:,indOut1),'zx') ...
%         *toolSp.coefs + toolPathPtRes(:,indOut1);
%     Q = fnval(toolSp1,uLimTmp(1,indOut1):0.01:uLimTmp(2,indOut1));
%     h1 = plot3(Q(1,:),Q(2,:),Q(3,:),'b');
%     hold on;
...
%         axesRot([0;0;1],[1;0;0],toolNormDirectRes(:,indOut1),toolCutDirectRes(:,indOut1),'zx') ...
%         *toolSp.coefs + toolPathPtRes(:,indOut1);
%     Q = fnval(toolSp2,uLimTmp(1,indOut1):0.01:uLimTmp(2,indOut1));
%     h2 = plot3(Q(1,:),Q(2,:),Q(3,:),'b');
%     hold on;

    angleOutRev = wrapToPi(angleOut + pi);
    angleDelRev = angleOutRev - angle(indOut1);
    [indOut2,indOut3] = getOuterLoopToolPathIndex(angleOutRev,angleDelRev,loopPtNumLast);
    [resTmp(1,indOut1),~,uLimTmpRev] = residual3D( ...
        toolPathPtRes,toolNormDirectRes,toolCutDirectRes,toolContactURes, ...
        toolSp,toolRadius,uLimTmp(:,indOut1),indOut1,indOut2,indOut3);
    uLimTmp(1,indOut1) = uLimTmpRev(2);
...
%         axesRot([0;0;1],[1;0;0],toolNormDirectRes(:,indOut1),toolCutDirectRes(:,indOut1),'zx') ...
%         *toolSp.coefs + toolPathPtRes(:,indOut1);
%     Q = fnval(toolSp3,uLimTmp(1,indOut1):0.01:uLimTmp(2,indOut1));
%     plot3(Q(1,:),Q(2,:),Q(3,:),'b');
%     hold on;
end
